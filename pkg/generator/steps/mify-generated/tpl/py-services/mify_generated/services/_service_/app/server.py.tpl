...
from {{.Workspace.MifyGeneratedCommonPackage}}.metrics.server import MetricsServer

from {{.Workspace.MifyGeneratedServicePackage}}.openapi import app
from {{.Workspace.MifyGeneratedServicePackage}}.core.service_context import MifyServiceContext


class ServerConf:
    API_ENDPOINT: str = '{{.Service.ApiEndpoint}}'
    MAINTENANCE_ENDPOINT: str = '{{.Service.MaintenanceEndpoint}}'

    _env_mapping: dict = {
        'API_ENDPOINT': '{{.Service.ApiEndpointEnv}}',
        'MAINTENANCE_ENDPOINT': '{{.Service.MaintenanceEndpointEnv}}',
    }
